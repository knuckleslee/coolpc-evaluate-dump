...
COVERED = re.compile(
    ...
    r"|\d{2,3}\s?%|TKL|中文|英文|2\.4\s?G|Bolt.*|Unifying.*|Omni.*|[48]K|Mac|.*軸"
    r"|\d{3,6}\s?dpi.*"
    r"|Wi-?Fi\s?[45678]E?|(AXE|AX|AC|BE)\s?\d{3,5}"
    r"|\d+(\.\d+)?\s?(Gbps|Gb)|\d+(/\d+)?\s?Mbps|\d+\s?埠"
    r"|\d+\s?VA|\d+\s?W|在線互動式?|離線式|在線式"
    r"|模擬正弦波|純正弦波(輸出)?|自動穩壓|.*突波.*|.*雷擊.*"
